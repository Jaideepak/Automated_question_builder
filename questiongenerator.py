...
class QuestionGenerator():
    ...
    def generate_questions_from_inputs(self, qg_inputs, num_questions):
       ...
       for qg_input in qg_inputs:
         if count < int(num_questions):
            ...
            # Clean the question by removing unwanted tokens like '<pad>' and '</s>'
            question = question.replace('pad>', '').replace('</s', '').strip()
            question = question.strip(punctuation)  # remove trailing punctuation
            question += "?"  # Ensure it ends with a question mark
    ...
    def _prepare_qg_inputs(self, sentences, text):
        ...
        for sentence in sentences:
            qg_input = '{} {} {} {}'.format(self.ANSWER_TOKEN, sentence, self.CONTEXT_TOKEN, text)
        ...
        for i in range(len(sentences)):
            ...
            if entities:
                for entity in entities:
                    qg_input = '{} {} {} {}'.format(self.ANSWER_TOKEN, entity, self.CONTEXT_TOKEN, sentences[i])
    ...
    def _get_MC_answers(self, correct_answer, docs):
        ...
        if len(matches) < num_choices:
            ...
            choices.extend(random.sample(list(pool), num_choices - len(choices)))  # Convert pool to list here
        else:
    ...
    def _encode_qg_input(self, qg_input):
        return self.qg_tokenizer(qg_input, pad_to_max_length=True, max_length=self.SEQ_LENGTH, truncation=True, return_tensors="pt").to(self.device)
    ...
    def _get_ranked_qa_pairs(self, generated_questions, qg_answers, scores, num_questions=10):
        ...
        for i in range(num_questions):
            index = scores[i]
            qa = self._make_dict(generated_questions[index].split('?')[0] + '?', qg_answers[index])
            qa_list.append(qa)
        return qa_list
    ...
    def _get_all_qa_pairs(self, generated_questions, qg_answers):
        ...
        for i in range(len(generated_questions)):
            qa = self._make_dict(generated_questions[i].split('?')[0] + '?', qg_answers[i])
            qa_list.append(qa)
        return qa_list
...
class QAEvaluator():
    ...
    def _encode_qa(self, question, answer):
        ...
        return self.qae_tokenizer(text=question, text_pair=correct_answer, pad_to_max_length=True, max_length=self.SEQ_LENGTH, truncation=True, return_tensors="pt")
...
def print_qa(qa_list, show_answers=True):
    for i in range(len(qa_list)):
        ...
        if type(answer) is list:
            if show_answers:
                print('{}A: 1.'.format(space), answer[0]['answer'], np.where(answer[0]['correct'], '(correct)', ''))
                for j in range(1, len(answer)):
                    print('{}{}.'.format(space + '   ', j + 1), answer[j]['answer'], np.where(answer[j]['correct'] == True, '(correct)', ''))
